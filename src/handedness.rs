...
pub trait Handedness: Clone + Copy + Debug + 'static {
    const FORWARD_Y_SIGN: f32;
    // `glam::const_vec3!` is deprecated  since 0.21 and to be replaced with
    // `glam::Vec3::new(0.0, Self::FORWARD_Z_SIGN, 0.0)`. Consider replacing
    // it when bumping glam's minimum version to 0.21.
    #[allow(deprecated)]
    const FORWARD: Vec3 = glam::const_vec3!([0.0, Self::FORWARD_Y_SIGN, 0.0]);
